function main(serPort)
    global cam_fov robot_radius cam_depth_range_ratio cam_depth_img_width cam_depth_img_center
    %init constants
    cam_fov = 74; %degrees
...
function obstacle_identified =identify_obstacle(Camera_Depth_Info)
    global cam_depth_img_width robot_radius
    detect_params = detect_object(Camera_Depth_Info);
...
function turn_state=avoid_obstacle(serPort, CameraHandle)
    ...
    global cam_depth_img_width
    [D, I] = get_camera_image(CameraHandle);
    detect_params = detect_object(D);
    near_left = detect_params.extrema.Extrema(8,1);
    near_right = detect_params.extrema.Extrema(4,1);
